...
def main():
    ...
    if len(model_lst) == 0:
        ...
        my_eff = efficientnet_b0(pretrained=True)
        model = Model.Model_Efficientnet(features=my_eff.features, bins=2).cuda()
    ...
    for img_id in ids:
        ...
        if FLAGS.show_yolo:
            ...
            cv2.imwrite(f'/content/with_yolo_{img_id}.png', numpy_vertical)
            #cv2.imshow('SPACE for next image, any other key to exit', numpy_vertical)
        else:
            cv2.imwrite(f'/content/3D-detection_{img_id}.png', img)
